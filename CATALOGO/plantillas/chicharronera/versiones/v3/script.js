...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (nombreProducto) {
        // Asumiendo que tu función de búsqueda o renderizado 
        // puede encontrar el producto por su nombre
...
async function compartirCatalogo(btn) {
    ...
    // 1. Intentar usar el menú nativo del sistema
    if (navigator.share) {
        try {
        ...
        // 2. Si no es compatible, ejecutamos la lógica de copiar manualmente
        copyToClipboard(btn);
    }
}

// Función para compartir un producto específico
...
function buscarYAbrirProducto(nombre) {
    // Supongamos que tu array de productos se llama 'productosData'
    const productoEncontrado = productosData.find(p => p.nombre === nombre);
    
    if (productoEncontrado) {
        // Llamas a la función que ya usas para abrir el modal
        // (La que rellena los datos y le pone .classList.add('active'))
